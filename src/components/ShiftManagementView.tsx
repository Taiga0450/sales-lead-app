...
export default function ShiftManagementView({
  ...
  const monthTotals = useMemo(() => {
    const map = new Map<string, { identity: string; name: string; hours: number }>();
    for (const entries of Object.values(calendar)) {
      for (const e of entries) {
        const cur = map.get(e.identity) ?? { identity: e.identity, name: e.name, hours: 0 };
        cur.hours += e.hours;
        if (e.name) cur.name = e.name;
        map.set(e.identity, cur);
      }
    }
  ...
  const grandTotalCost = monthTotals.reduce((sum, p) => sum + p.hours * (wages[p.identity] ?? 0), 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
          <p className="text-xs font-medium text-foreground/50">今月のバイト人件費（概算）</p>
          <p className="mt-2 text-3xl font-bold text-emerald-600">{yen.format(grandTotalCost)}</p>
          <p className="mt-1 text-[11px] text-foreground/40">時給が未設定の人は0円で計算されます</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <div className="border-b border-border bg-brand-light/40 px-5 py-3">
          <h2 className="font-bold">スタッフ別 稼働時間・時給・人件費</h2>
        </div>
        ...
        <div className="flex flex-col">
          {monthTotals.map((p) => {
            ...
            return (
              <div key={p.identity} className="grid grid-cols-5 items-center gap-3 border-b border-border px-5 py-3 text-sm last:border-0">
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    className="w-24 rounded-md border border-border px-2 py-1 text-sm outline-none focus:border-brand"
                  />
                  <span className="text-xs text-foreground/40">円/h</span>
                </div>
                <span className="font-semibold text-emerald-600">{yen.format(p.hours * wage)}</span>
                <div>
                  {hasDraft && (
                    <button
                      type="button"
